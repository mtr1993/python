...
class Conf:
    hosts = '10.68.112.13,10.68.112.14,10.68.112.15'
    user = 'aihc'
    port = 12688
    password = 'aihc'
    database = 'bhc'
...
def get_domain_hosts(connect, domain):
    ...
    sql = "select * from (select * from (select a.node_code,a.host_address,IFNULL(b.project,''),IFNULL(b.usage,'')," \
          "IFNULL(b.cost,0),IFNULL(b.manager,'')," \
          "IFNULL(b.opendate,''),IFNULL (b.version,''),IFNULL(b.core,0),IFNULL(b.memory,0)," \
          "IFNULL(b.space,0),b.area,a.DOMAIN_ID from pm_host_daemon_node a " \
          "left join pm_host_property b on a.host_address = b. address order by a.node_code) c " \
          "where c.node_code <> 'patrol' and c.DOMAIN_ID <= %d ) d order by substring(d.host_address ,11,3)" % (
        domain)
    cursor.execute(sql)
...
def get_partol_script_info(script_file, task_name, db_connect):
    ...
    try:
        ...
        sql = "select TASK_ID,TASK_TYPE, SCRIPT_TYPE, SCRIPT_FILE," \
              "TIME_OUT from pm_kpi_collect_task where PARENT_ID = " \
              "(select task_id from pm_kpi_collect_task where script_file = '%s') and task_name = '%s'" % (
...
def get_host_apps(publisher, hosts, db_connect):
    ...
    try:
        ...
        for message in messages:
            if message is not None:
                try:
                    ...
                    logger.info(f"message.get_headers()  is {message.get_headers()}")
                    logger.info(f"body is {result}")
                except Exception:
        ...
        executor.end()
    logger.info(f'host_apps is {host_apps}')
    return host_apps


def get_host_mongo_query_apps(publisher, hosts, db_connect):
    host_mongo_query_apps = []

    executor = ScriptExecutor(publisher)
    logger.info(f"Conf.script_file is {Conf.script_file}")
    script_id, script_name, script_type, timeout = get_partol_script_info(Conf.script_file, 'host_mongo_query_apps', db_connect)
    logger.info(f"script_id is {script_id}, script_name is {script_name}, script_type is {script_type}, timeout is {timeout}")
    try:
        executor.begin()
        ...
        for message in messages:
            if message is not None:
                try:
                    # mongo_apps = []
                    # ip = message.get_header_value('performer')
                    # one_host_result = message.get_body()
                    # mongo_apps.append(one_host_result)
                    # apps_dic = {'ip': ip}
                    # apps_dic['apps'] = mongo_apps
                    # host_mongo_query_apps.append(apps_dic)
                    # mongo_apps = []
                    # result = JSON.loads(str(message.get_body()).replace('IP',message.get_header_value('performer')))
                    result = JSON.loads(message.get_body())
                    host_mongo_query_apps.append(result)
                    logger.info(f"message.get_headers()  is {message.get_headers()}")
                    logger.info(f"message.get_body() is {message.get_body()}")
                except Exception:
                    logger.error(f"message.get_headers()  is {message.get_headers()}")
                    logger.error(f"message.get_body() is {message.get_body()}")
                    continue

                # host_apps[message.get_header_value('performer')] = result
    finally:
        executor.end()
    host_mongo_query_apps_sort = sorted(host_mongo_query_apps, key=lambda x: x['ip'], reverse=False)
    logger.info(f'host_mongo_query_apps is {host_mongo_query_apps_sort}')

    return host_mongo_query_apps_sort
...
def get_excep_result():
    ...
    excep_result = sorted(excep_result_value, key=lambda x: x['Ip'], reverse=False)
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info(f'start  host_mongo_query_apps ')
        host_mongo_query_apps = get_host_mongo_query_apps(pub, hosts, db_connect)

        logger.info(f'start  get_excep_result ')
        excep_result = get_excep_result()
        ...
        chapter.add_context({'date_cycle': date_cycle, 'table': hosts})
        ...
        host_mongo_query_apps_context = {'date_cycle': datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                         'table': host_mongo_query_apps}
        chapter.add_context(host_mongo_query_apps_context)
        logger.info(f"host_mongo_query_apps table  is {host_mongo_query_apps_context}")
        ...
        table = {'table': excep_result, 'date_cycle': str(yesterday) + "-" + str(now_day)}
        chapter.add_context(table)
        logger.debug(f"host-mongo_query_exceptions table  is {table}")
        ...
        for host in hosts:
            ...
            if app_info is not None:
                ...
                table = {"table": app_info}
                logger.info(f"app_info table  is {table}")
        ...
        local = os.path.join('/data/mongo/bhc/robot-node/.toolkit/doc_1_20190807', file_name)
        # 远程下载文件主机目录配置
        ...
        logger.info(f'success  file is {file_name}')
    finally:
        db_connect.close()
        pub.disconnect()

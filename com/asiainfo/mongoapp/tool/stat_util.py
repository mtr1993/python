...
def insert_stat(mongo_writer, mongo_client, stat_db_name, stat_coll_name, filename, file_modify_time_from_path, ip):
...
# update stat
def update_stat(mongo_writer, mongo_client, stat_db_name, stat_coll_name, filename, line_number, file_modify_time_from_path, ip):
    end_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
    status = 'finish'
    condition = {"FileName": filename, 'Ip': ip}
